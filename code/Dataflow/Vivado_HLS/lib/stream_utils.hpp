...
template<unsigned int DataWidth, unsigned int numBytes>
void Stream2Mem(hls::stream<ap_uint<DataWidth> > & in,volatile  ap_uint<DataWidth> * out) {
...
void Streaming_pad_basic(hls::stream<ap_uint<NumChannels* In_t::width> > &in,
...
void Streaming_pad(hls::stream<ap_uint<vals_per_input* In_t::width> > &in,
		hls::stream<ap_uint<vals_per_output* In_t::width> > &out){
#pragma HLS INLINE
	hls::stream<ap_uint<NumChannels* In_t::width>> depth_packed_stream("depth packed stream");
	hls::stream<ap_uint<NumChannels* In_t::width>> depth_pp_stream("depth pack&pad stream");
...
#if !__SYNTHESIS__
	assert( NumChannels%vals_per_output == 0);
#endif

	if(NumChannels!= vals_per_output)
	{
		//This is IFM_dim before padding???
		Streaming_pad_basic<ImgDim,KernelDim,Stride,NumChannels,
							In_t, PaddingStyle>(depth_packed_stream,depth_pp_stream);

		StreamingDataWidthConverter_Batch<NumChannels* In_t::width,vals_per_output* In_t::width,
		((ImgDim+2)*(ImgDim+2))>(depth_pp_stream, out, 1);
	}
	else{
		//This is IFM_dim before padding???
		Streaming_pad_basic<ImgDim,KernelDim,Stride,NumChannels,
							In_t, PaddingStyle>(depth_packed_stream,out);
	}
