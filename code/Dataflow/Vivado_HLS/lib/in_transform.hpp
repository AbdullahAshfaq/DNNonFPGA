...
>
void in_transform(hls::stream<ap_uint<8*next_SIMDi*4*4>> &in, hls::stream<ap_uint<trans_bit*next_SIMDi*4*4>> &out)
{
...
ap_uint<trans_bit*next_SIMDi*4*4> outElem = 0;
	...
	for(unsigned i=0; i<(IFMdim/2 - 1)*(IFMdim/2 - 1)*IFM_ch/next_SIMDi; i++)
	{
